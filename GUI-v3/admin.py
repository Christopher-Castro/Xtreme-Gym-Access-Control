...
from aws.rekognition import delete_face
...
class InstructorControls:
    ...
    def delete_user(self):
        if not hasattr(self, 'chosenRow'):
            messagebox.showerror("Error!", "No se seleccionó ningún cliente.")
            return
        
        print(self.chosenRow[0])
        
        try:
            delete_face(self.chosenRow[0])
        except Exception as e:
            print(e)
            messagebox.showerror("Error!", "error al borrar registro")
            return

        messagebox.showinfo("Success!", "Registro borrado exitosamente")
    ...
    def instructorFrameButtons(self):
        ...
        self.btnBook = Button(self.entriesFrame, command=self.delete_user, text="Borrar Registro", bd=0, cursor="hand2",
                              bg="#ff1909",
                              fg="black", width=15, font=("Impact", 15))
        self.btnBook.grid(row=0, column=4, padx=0)
